         ...
         */
            ...
            SelfMessage *message = check_and_cast<SelfMessage *>(msg); //强制类型转换
                ...
                if(neighborLength > 0){ //该节点没有邻居时，则不进行处理
                     ...
                     */
                    if (candidate.size() > 0) {
                        divideGroup2(getIndex(),0.8);
                        for(i = 0;i < K;i++){
                            Suc_Rate_GROUP[i] = Lq[nodefrom][neighbor[i]];
                        }
...
void Job1::finish(int transmissions, int hop, double sumEnergy) {
    ...
    ofstream f("D:\\A-WorkSpace\\Omnet++\\data\\LDC_ACOR.txt", ios::app);
    EV << "LDC-ACOR: \n";
