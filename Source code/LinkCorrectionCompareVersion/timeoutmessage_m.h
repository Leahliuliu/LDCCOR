//
// Generated file, do not edit! Created by nedtool 4.6 from timeoutmessage.msg.
//

#ifndef _TIMEOUTMESSAGE_M_H_
#define _TIMEOUTMESSAGE_M_H_

#include <omnetpp.h>

// nedtool version check
#define MSGC_VERSION 0x0406
#if (MSGC_VERSION!=OMNETPP_VERSION)
#    error Version mismatch! Probably this file was generated by an earlier version of nedtool: 'make clean' should help.
#endif



/**
 * Class generated from <tt>timeoutmessage.msg:2</tt> by nedtool.
 * <pre>
 * message timeoutMsg //超时消息
 * {
 *     bool finish; //结束
 *     int finishIndex; //结束的下标
 *     int dutyId;
 *     int nexthop; //下一跳
 *     int nodefrom;
 *     double beginTime;
 *     double endTime;
 * 
 *     int group[50]; //三个center不太明白
 *     int groupLength;
 * 
 *     int trace[50]; //轨迹记录
 *     int traceLength; //轨迹长度（跳数）
 * 
 *     int flag;
 * }
 * </pre>
 */
class timeoutMsg : public ::cMessage
{
  protected:
    bool finish_var;
    int finishIndex_var;
    int dutyId_var;
    int nexthop_var;
    int nodefrom_var;
    double beginTime_var;
    double endTime_var;
    int group_var[50];
    int groupLength_var;
    int trace_var[50];
    int traceLength_var;
    int flag_var;

  private:
    void copy(const timeoutMsg& other);

  protected:
    // protected and unimplemented operator==(), to prevent accidental usage
    bool operator==(const timeoutMsg&);

  public:
    timeoutMsg(const char *name=NULL, int kind=0);
    timeoutMsg(const timeoutMsg& other);
    virtual ~timeoutMsg();
    timeoutMsg& operator=(const timeoutMsg& other);
    virtual timeoutMsg *dup() const {return new timeoutMsg(*this);}
    virtual void parsimPack(cCommBuffer *b);
    virtual void parsimUnpack(cCommBuffer *b);

    // field getter/setter methods
    virtual bool getFinish() const;
    virtual void setFinish(bool finish);
    virtual int getFinishIndex() const;
    virtual void setFinishIndex(int finishIndex);
    virtual int getDutyId() const;
    virtual void setDutyId(int dutyId);
    virtual int getNexthop() const;
    virtual void setNexthop(int nexthop);
    virtual int getNodefrom() const;
    virtual void setNodefrom(int nodefrom);
    virtual double getBeginTime() const;
    virtual void setBeginTime(double beginTime);
    virtual double getEndTime() const;
    virtual void setEndTime(double endTime);
    virtual unsigned int getGroupArraySize() const;
    virtual int getGroup(unsigned int k) const;
    virtual void setGroup(unsigned int k, int group);
    virtual int getGroupLength() const;
    virtual void setGroupLength(int groupLength);
    virtual unsigned int getTraceArraySize() const;
    virtual int getTrace(unsigned int k) const;
    virtual void setTrace(unsigned int k, int trace);
    virtual int getTraceLength() const;
    virtual void setTraceLength(int traceLength);
    virtual int getFlag() const;
    virtual void setFlag(int flag);
};

inline void doPacking(cCommBuffer *b, timeoutMsg& obj) {obj.parsimPack(b);}
inline void doUnpacking(cCommBuffer *b, timeoutMsg& obj) {obj.parsimUnpack(b);}


#endif // ifndef _TIMEOUTMESSAGE_M_H_

